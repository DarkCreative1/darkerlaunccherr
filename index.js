  ...
  files.forEach(f => {
    readdir("./src/commands/" + f, (err2, files2) => {
      ...
      files2.forEach(file => {
        ...
        console.log(`[TheTomenTosa Dev.- Komutlar] ${prop.name} yüklendi!`);
...
readdir("./src/events", (err, files) => {
  ...
  files.filter((file) => file.endsWith(".js")).forEach((file) => {
    ...
    console.log(`[TheTomenTosa Dev. - Event] ${prop.conf.name} yüklendi!`);
  });
});

client.login( MTE3MTUwODIzNDM0Mzk0NDMxNQ.G6hE0Q.vn3A-g3ZBC6h3dFqs0Rx66PJsyUpEDvQmyPP7o.TOKEN)
